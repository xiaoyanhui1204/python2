...
# ------------------------  华丽的分割线  ------------------------ #
    ...
    # 输入卡号
    def __input_cardid(self, timp):
        cardid = input(f'请输入要{timp}的银行卡号:')
    ...
    def save_money(self):
        ...
        # 4.self.__save_users()
        user1 = self.__input_cardid('存款')
        if not user1:
            print('=> 卡号不存在.')
            return
        passwd = input('请输入银行卡密码:')
        if passwd != user1.card.passwd:
            print('=> 密码错误.')
            return
        moneys = float(input('请输入存款金额:'))
        user1.card.money += moneys
        print('当前余额:', user1.card.money)
        self.__save_users()
    ...
    def get_money(self):
        ...
        # 4.self.__save_users()
        user2 = self.__input_cardid('取款')
        if not user2:
            print('=> 卡号不存在.')
            return
        passwd = input('请输入银行卡密码:')
        if passwd != user2.card.passwd:
            print('=> 密码错误.')
            return
        moneys = float(input('请输入取款金额:'))
        while True:
            if user2.card.money < moneys:
                print('=>余额不足')
                continue
            user2.card.money -= moneys
            break
        self.__save_users()
    ...
    def transform_money(self):
        ...
        # 5.self.__save_users()
        user3 = self.__input_cardid('转出')
        user4 = self.__input_cardid('对方')
        money = float(input('请输入转账金额:'))
        if not user3:
            print('=> 卡号不存在.')
            return
        passwd = input('请输入银行卡密码:')
        if passwd != user3.card.passwd:
            print('=> 密码错误.')
            return
        user3.card.money -= money
        user4.card.money += money
        self.__save_users()
    ...
    def modify_password(self):
        ...
        if not user:
            ...
            return
        while True:
            idcard_1 = input('请输入身份证:')
            if idcard_1 != user.idcard:
                print('=>身份证错误')
                continue
            break
    ...
    def lock_card(self):
        ...
        # 4.self.__save_users()
        user6 = self.__input_cardid('锁卡')
        if not user6:
            print('=> 卡号不存在.')
            return
        passwd = input('请输入银行卡密码:')
        if passwd != user6.card.passwd:
            print('=> 密码错误.')
            return
        user6.card.islock = True
        self.__save_users()
    ...
    def unlock_card(self):
        ...
        # 4.self.__save_users()
        user7 = self.__input_cardid('解锁')
        if not user7:
            print('=> 卡号不存在.')
            return
        passwd = input('请输入银行卡密码:')
        if passwd != user7.card.passwd:
            print('=> 密码错误.')
            return
        user7.card.islock = False
        self.__save_users()
